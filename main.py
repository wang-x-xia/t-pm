...
from src.module import check_module_data, check_module_files, visualize_module_dependencies, check_module_dependencies, analyze_module_dependencies, check_method_files
...
def check_all_data(show_success=False):
    """检查项目内所有数据的合理性"""
    # 收集所有检查结果和明细信息
    check_results = {
        "概念管理数据": {
    ...
    # 检查方法文件
    method_files_result, method_files_success, method_files_failure = check_method_files()
    check_results["方法文件"]["状态"] = "通过" if method_files_result else "失败"
    if show_success:
        check_results["方法文件"]["成功"] = method_files_success
    check_results["方法文件"]["失败"] = method_files_failure

    # 生成最终的YAML结果
    final_result = {
        "检查结果": check_results,
        "整体状态": "通过" if all([concept_result, background_result, method_result, module_result, requirement_result, process_result, module_files_result, module_deps_result, req_story_result, method_files_result]) else "失败"
    }

    # 输出YAML格式结果
    print(yaml.dump(final_result, allow_unicode=True, sort_keys=False))

    return all([concept_result, background_result, method_result, module_result, requirement_result, process_result, module_files_result, module_deps_result, req_story_result, method_files_result])
...
def main():
    ...
    # 检查命令
    check_parser = subparsers.add_parser('check', help='检查所有数据的合理性')
    check_parser.add_argument('--show-success', '-s',
                              action='store_true', help='显示成功信息')

    # 显示命令
    show_parser = subparsers.add_parser('show', help='显示相关信息')
    show_subparsers = show_parser.add_subparsers(
        dest='show_command', help='显示命令')
    ...
    # 创建背景
    background_parser = create_subparsers.add_parser(
        'background', help='创建新背景')
    ...
    # 创建需求
    requirement_parser = create_subparsers.add_parser(
        'requirement', help='创建新需求')
    ...
    apply_subparsers = apply_parser.add_subparsers(
        dest='apply_command', help='应用命令')

    # 应用方法
    apply_method_parser = apply_subparsers.add_parser(
        'method', help='应用方法到特定上下文')
    ...
    visualize_subparsers = visualize_parser.add_subparsers(
        dest='visualize_command', help='可视化命令')

    # 可视化流程
    visualize_process_parser = visualize_subparsers.add_parser(
        'process', help='可视化展示流程')
    ...
    if args.command == 'check':
        check_all_data(args.show_success)
        ...
        if args.create_command == 'process':
            create_process(args.name, args.background,
                           args.inputs, args.method, args.result)
            ...
            create_user_story(args.requirement, args.name,
                              args.description, args.process)
